...
def _maybe_inverse_transform(scaler, data):
    """Inverse-transform `data` using scaler but handle the case where
    scaler.mean/std have more channels than data's last dim (slice to match).
    Works for numpy arrays and TensorFlow tensors.
    """
    if scaler is None:
        return data
    ...
    # If data is a TF tensor
    if isinstance(data, tf.Tensor):
        # Try to get static last-dim
        static_last = data.get_shape().as_list()[-1]
        if static_last is not None and hasattr(mean, '__len__') and len(mean) != static_last:
            mean_use = np.array(mean)[:static_last]
            std_use = np.array(std)[:static_last]
        else:
            mean_use = np.array(mean)
            std_use = np.array(std)
        mean_t = tf.constant(mean_use, dtype=data.dtype)
        std_t = tf.constant(std_use, dtype=data.dtype)
        return data * std_t + mean_t

    # numpy array
    if hasattr(data, 'shape'):
        last = data.shape[-1]
        if hasattr(mean, '__len__') and len(mean) != last:
            mean_use = np.array(mean)[:last]
            std_use = np.array(std)[:last]
        else:
            mean_use = np.array(mean)
            std_use = np.array(std)
        return (data * std_use) + mean_use
...
def masked_mse_loss(scaler, null_val):
    def loss(preds, labels):
        if scaler:
            preds = _maybe_inverse_transform(scaler, preds)
            labels = _maybe_inverse_transform(scaler, labels)
